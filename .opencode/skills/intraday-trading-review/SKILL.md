...
**日期约定:** 复盘的 strategy date 是昨天 (T)，verification 文件写入 `memory/daily/{T}/intraday_verification.md`。
...
**Output:** `memory/daily/{YYYY}-{MM}-{DD}/intraday_verification.md`
...
|------|---------|--------------|
| `memory/daily/{date}/intraday_verification.md` | Detailed intraday verification | Step 3+4 |
...
| 4 | Write Memory | Analysis report | `memory/daily/{date}/intraday_verification.md` + INDEX/RULES updates |
...
- **AFTER** generating review:
  - Write verification to `memory/daily/{YYYY-MM-DD}/intraday_verification.md` (使用策略日期)
