...
from build_step2_timing import update_report
from build_theme_evidence_input import build_input as build_theme_evidence_input
from build_theme_stocks_base import build_filtered_doc, load_extra_stocks, load_universe, theme_library_dir
from build_theme_stocks_universe import collect_theme_specs_from_json, enrich_structured_source_flags
from build_theme_stocks_base import build_doc_from_themes, universe_doc
from mapper_json_lib import (build_deterministic_mapper_base, default_predict_dir, load_pool,
                             ensure_doc_date, load_trading_scope, read_json, write_json, publish_theme_stocks)
from validate_theme_stocks_json import check_doc as check_theme_stocks_doc


def market_views_for(themes: list[dict[str, Any]]) -> dict[str, Any]:
    script = theme_library_dir() / "scripts" / "query_theme.py"
    spec = importlib.util.spec_from_file_location("theme_query_for_daily_mapping", script)
    ...
    result = {}
    for theme in themes:
        data = module._resolve_theme(theme["name"])
        if data:
            result[theme["name"]] = module._compute_market_view(data, top=10)
...
def main() -> int:
    ...
    try:
        ...
        if not args.reuse_pool:
            command = [sys.executable, str(Path(__file__).parent / "fetch_pool_indicators.py"),
                       "--codes-file", str(universe_path), "--json", "-o", str(pool_path)]
            completed = subprocess.run(command, check=False)
            if completed.returncode:
                raise RuntimeError(f"indicator fetch failed with exit code {completed.returncode}")
