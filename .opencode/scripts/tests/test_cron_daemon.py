...
SCRIPT = Path(__file__).resolve().parents[1] / "cron-daemon.py"
SPEC = importlib.util.spec_from_file_location("cron_daemon", SCRIPT)
cron_daemon = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = cron_daemon
assert SPEC.loader is not None
SPEC.loader.exec_module(cron_daemon)
...
class ConfigTests(unittest.TestCase):
    def test_default_config_loads_four_tasks_and_overrides_defaults(self):
        config = cron_daemon.load_config(cron_daemon.DEFAULT_CONFIG_PATH)
        self.assertEqual(
