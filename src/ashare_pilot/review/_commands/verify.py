...
from ashare_pilot.indicators._commands.calculate import calc_atr, calc_sma, parse_value
...
def main(argv=None) -> None:
    ...
    strategy_json_path = Path(args.strategy_json) if args.strategy_json else workspace_path("predict", args.date, "strategy.json")
    ...
    if not codes and not strategy_json_path:
        strategy_path = Path(args.strategy) if args.strategy else workspace_path("predict", args.date, "strategy.md")
    output_path = Path(args.output) if args.output else workspace_path("memory", "daily", args.date, "verification.json")
    pool_path = Path(args.pool_indicators) if args.pool_indicators else workspace_path("predict", args.date, "pool_indicators.json")
