...
COARSE_WAIT_SECONDS = 60.0
FINE_WAIT_SECONDS = 1.0
FINE_WAIT_WINDOW_SECONDS = 60.0
...
def run_task(
    ...
    log_root = task_log_root or workspace_path("logs", "cron-tasks")
    log_dir = log_root / run.scheduled_at.date().isoformat()
...
def daemon_loop(
    ...
    while True:
        ...
        _wait_until(scheduled_at, calendar.timezone)
        for run in batch:
...
def _wait_until(
    ...
    *,
    ...
    """Wait for a wall-clock deadline while tolerating clock corrections."""
    if now_fn is None:
        now_fn = lambda: datetime.now(timezone)
    if sleep_fn is None:
        sleep_fn = time.sleep

    while True:
        remaining = (scheduled_at - now_fn()).total_seconds()
        if remaining <= 0:
            return
        interval = (
            COARSE_WAIT_SECONDS
            if remaining > FINE_WAIT_WINDOW_SECONDS
            else FINE_WAIT_SECONDS
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        help="scheduler config path (default: <workspace>/config/cron-tasks.json)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger = setup_logger(workspace_path("logs", "cron-daemon.log"))
    try:
        config_path = _config_path_from_argument(args.config) if args.config else workspace_path("config", "cron-tasks.json")
        config = load_config(config_path)
        calendar = TradingCalendar(config.calendar_path)
        now = datetime.now(calendar.timezone)
        logger.info("PROJECT_DIR: %s", workspace_path())
