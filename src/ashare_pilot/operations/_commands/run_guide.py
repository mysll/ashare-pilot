...
from ashare_pilot.operations.operation_time import MARKET_TZ, now_market, parse_market_datetime
from ashare_pilot.operations._commands.validate_snapshot import validate as validate_snapshot
...
def eligible_snapshots(operation_dir: Path, trade_date: str, before: datetime) -> list[tuple[datetime, Path, dict[str, Any]]]:
    ...
    for path in immutable_snapshot_paths(operation_dir):
        ...
        if doc is None or doc.get("date") != trade_date or validate_snapshot(
            doc, reference_base=workspace_path(), require_references=True
        ):
            continue
...
def run_command(command: list[str]) -> None:
    completed = subprocess.run(command, cwd=workspace_path(), check=False)
...
def build_current_artifacts(
    *,
    ...
    while True:
        command = [
            sys.executable,
            "-m", "ashare_pilot", "--workspace", str(workspace_path()),
    ...
    run_command([
        sys.executable, "-m", "ashare_pilot", "--workspace", str(workspace_path()),
        "operations", "snapshot", "validate", str(snapshot_path)
    ])
    ...
    run_command([
        sys.executable, "-m", "ashare_pilot", "--workspace", str(workspace_path()),
        "operations", "decision", "validate", str(pending_decision)
    ])
...
def main(argv=None) -> int:
    ...
    operation_dir = workspace_path("operation", trade_date)
